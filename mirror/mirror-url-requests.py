...
with open(global_url_list) as file:
  for line in file:
    ...
    url_directory = f"{global_uvi_url_downloads}/{url_hash_1}/{url_hash_2}/{url_hash_3}/{url_hash_4}/{url_hash}"
    ...
    if not already_seen:
      ...
      response_file = f"{url_directory_raw_data}/server_response.data"
      with open(response_file, "wb") as f:
        f.write(response.content)
            #
            # Request file data
            #
      request_data_file = f"{url_directory}/request.json"
      request_data = {
          "URL_requested": url,
      ...
      if os.path.exists(request_data_file_txt):
          os.remove(request_data_file_txt)
            ...
            # Response file data
            #
      response_data_file = f"{url_directory}/response.json"
      with open(response_data_file, "w") as f:
        ...
        f.write(json.dumps(response_data, indent=4, sort_keys=True))
            ...
            # Remove old txt file if exists
            #
      response_data_file_txt = f"{url_directory}/response.txt"
